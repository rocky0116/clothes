 ...
 * 
 * Author: Luke Hutchison (luke.hutch@mit.edu) Please drop me an email if you use this code so I can
 ...
 *   </code>
 * 
 * Changelog: 2010-06-09 v1.5.1 Some API changes to make it possible to selectively update or not
 * update scale / rotation. Fixed anisotropic zoom. Cleaned up rotation code. Added more comments.
 * Better var names. (LH) 2010-06-09 v1.4 Added ability to track pinch rotation (Mickael Despesse,
 * author of "Face Frenzy") and anisotropic pinch-zoom (LH) 2010-06-09 v1.3.3 Bugfixes for
 * Android-2.1; added optional debug info (LH) 2010-06-09 v1.3 Ported to Android-2.2 (handle
 * ACTION_POINTER_* actions); fixed several bugs; refactoring; documentation (LH) 2010-05-17 v1.2.1
 * Dual-licensed under Apache and GPL licenses 2010-02-18 v1.2 Support for compilation under Android
 * 1.5/1.6 using introspection (mmin, author of handyCalc) 2010-01-08 v1.1.1 Bugfixes to Cyanogen's
 * patch that only showed up in more complex uses of controller (LH) 2010-01-06 v1.1 Modified for
 * official level 5 MT API (Cyanogen) 2009-01-25 v1.0 Original MT controller, released for hacked G1
 * kernel (LH)
 * 
 * Planned features: - Add inertia (flick-pinch-zoom or flick-scroll)
 * 
 * Known usages: - Mickael Despesse's "Face Frenzy" face distortion app, to be published to the
 * Market soon - Yuan Chin's fork of ADW Launcher to support multitouch - David Byrne's fractal
 * viewing app Fractoid - mmin's handyCalc calculator - My own "MultiTouch Visualizer 2" in the
 * Market - Formerly: The browser in cyanogenmod (and before that, JesusFreke), and other firmwares
 * like dwang5. This usage has been replaced with official pinch/zoom in Maps, Browser and
 * Gallery[3D] as of API level 5.
 * 
...
/**
 * A class that simplifies the implementation of multitouch in applications. Subclass this and read
 * the fields here as needed in subclasses.
 * 
 */
public class MultiTouchController<T> {

	/**
	 * Time in ms required after a change in event status (e.g. putting down or lifting off the
	 * second finger) before events actually do anything -- helps eliminate noisy jumps that happen
	 * on change of status
	 */
	private static final long	EVENT_SETTLE_TIME_INTERVAL		= 20;

	/**
	 * The biggest possible abs val of the change in x or y between multitouch events (larger dx/dy
	 * events are ignored) -- helps eliminate jumps in pointer position on finger 2 up/down.
	 */
	 ...
	 * multitouch events (larger-jump events are ignored) -- helps eliminate jumps in pointer
	 * position on finger 2 up/down.
	 */
	...
	/**
	 * Extract fields from mCurrPt, respecting the update* fields of mCurrPt. This just avoids code
	 * duplication. I hate that Java doesn't support higher-order functions, tuples or multiple
	...
	/**
	 * Whether to handle single-touch events/drags before multi-touch is initiated or not; if not,
	 * they are handled by subclasses
	 */
	...
	/**
	 * Two or more touch points down, stretching/rotating an object using the first two touch
	 * points.
	 */
	...
	/**
	 * Whether to handle single-touch events/drags before multi-touch is initiated or not; if not,
	 * they are handled by subclasses. Default: true
	 */
		...
		try {
			...
			for (int histIdx = 0; histIdx <= histLen; histIdx++) {
				...
				if (!multiTouchSupported || pointerCount == 1) {
					// Use single-pointer methods -- these are needed as a special case (for some
					// weird reason) even if
					// multitouch is supported but there's only one touch point down currently --
					// event.getX(0) etc. throw
					...
					for (int ptrIdx = 0; ptrIdx < numPointers; ptrIdx++) {
						...
						// N.B. if pointerCount == 1, then the following methods throw an array
						// index out of range exception,
						// and the code above is therefore required not just for Android 1.5/1.6 but
						// also for when there is
						// only one touch point on the screen -- pointlessly inconsistent :(
						xVals[ptrIdx] = (Float) (processingHist ? m_getHistoricalX.invoke(event,
							ptrIdx, histIdx) : m_getX.invoke(event, ptrIdx));
						yVals[ptrIdx] = (Float) (processingHist ? m_getHistoricalY.invoke(event,
							ptrIdx, histIdx) : m_getY.invoke(event, ptrIdx));
	...
	/**
	 * Drag/stretch/rotate the selected object using the current touch position(s) relative to the
	 * anchor position(s).
	 */
	private void performDragOrPinch() {
		...
		// Set the new obj coords, scale, and angle as appropriate (notifying the subclass of the
		// change).
	...
	/**
	 * State-based controller for tracking switches between no-touch, single-touch and multi-touch
	 * situations. Includes logic for cleaning up the event stream, as events around touch up/down
	 * are noisy at least on early Synaptics sensors.
	 */
		...
		switch (mMode) {
			case MODE_NOTHING:
				...
				if (mCurrPt.isDown()) {
					...
					if (selectedObject != null) {
						...
						// Don't need any settling time if just placing one finger, there is no
						// noise
			...
			case MODE_DRAG:
				...
				if (!mCurrPt.isDown()) {
					...
					mMode = MODE_PINCH;
					// Restart the drag with the new drag position (that is at the midpoint between
					// the touchpoints)
					anchorAtThisPositionAndScale();
					// Need to let events settle before moving things, to help with event noise on
					// touchdown
					...
					// Point 1 is still down and point 2 did not change state, just do single-point
					// drag to new location
					if (mCurrPt.getEventTime() < mSettleEndTime) {
						// Ignore the first few events if we just stopped stretching, because if
						// finger 2 was kept down while
						// finger 1 is lifted, then point 1 gets mapped to finger 2. Restart the
						// drag from the new position.
						anchorAtThisPositionAndScale();
					}
					else {
			...
			case MODE_PINCH:
				...
				if (!mCurrPt.isMultiTouch() || !mCurrPt.isDown()) {
					...
					if (!mCurrPt.isDown()) {
						...
						anchorAtThisPositionAndScale();
						// Ignore the first few events after the drop, in case we dropped finger 1
						// and left finger 2 down
						mSettleStartTime = mCurrPt.getEventTime();
						mSettleEndTime = mSettleStartTime + EVENT_SETTLE_TIME_INTERVAL;
					}
	...
	/**
	 * A class that packages up all MotionEvent information with all derived multitouch information
	 * (if available)
	 */
	public static class PointInfo {
		...
		// Whether or not there is at least one finger down (isDown) and/or at least two fingers
		// down (isMultiTouch)
		...
		/**
		 * Copy all fields from one PointInfo class to another. PointInfo objects are volatile so
		 * you should use this if you want to keep track of the last touch event in your own code.
		 */
		...
		/**
		 * Calculate the squared diameter of the multitouch event, and cache it. Use this if you
		 * don't need to perform the sqrt.
		 */
		...
		/**
		 * Calculate the diameter of the multitouch event, and cache it. Uses fast int sqrt but
		 * gives accuracy to 1/16px.
		 */
		...
		/**
		 * Calculate the angle of a multitouch event, and cache it. Actually gives the smaller of
		 * the two angles between the x axis and the line between the two touchpoints, so range is
		 * [0,Math.PI/2]. Uses Math.atan2().
		 */
		...
		/**
		 * Return the X coord of the first touch point if there's only one, or the midpoint between
		 * first and second touch points if two or more.
		 */
		public float getX() {
			return xMid;
		}
		...
		/**
		 * Return the array of pointer ids -- only the first getNumTouchPoints() of these is
		 * defined. These don't have to be all the numbers from 0 to getNumTouchPoints()-1
		 * inclusive, numbers can be skipped if a finger is lifted and the touch sensor is capable
		 * of detecting that that particular touch point is no longer down. Note that a lot of
		 * sensors do not have this capability: when finger 1 is lifted up finger 2 becomes the new
		 * finger 1. However in theory these IDs can correct for that. Convert back to indices using
		...
		/**
		 * Return the pressure the first touch point if there's only one, or the average pressure of
		 * first and second touch points if two or more.
		 */
	...
	/**
	 * A class that is used to store scroll offsets and scale information for objects that are
	 * managed by the multitouch controller
	 */
	public static class PositionAndScale {
		...
		/**
		 * Set position and optionally scale, anisotropic scale, and/or angle. Where if the
		 * corresponding "update" flag is set to false, the field's value will not be changed during
		 * a pinch operation. If the value is not being updated *and* the value is not used by the
		 * client application, then the value can just be zero. However if the value is not being
		 * updated but the value *is* being used by the client application, the value should still
		 * be specified and the update flag should be false (e.g. angle of the object being dragged
		 * should still be specified even if the program is in "resize" mode rather than "rotate"
		 * mode).
		 */
		...
		/**
		 * Set position and optionally scale, anisotropic scale, and/or angle, without changing the
		 * "update" flags.
		 */
	...
	public static interface MultiTouchObjectCanvas<T> {
		...
		/**
		 * See if there is a draggable object at the current point. Returns the object at the point,
		 * or null if nothing to drag. To start a multitouch drag/stretch operation, this routine
		 * must return some non-null reference to an object. This object is passed into the other
		 * methods in this interface when they are called.
		 * 
		 * @param touchPoint
		 *            The point being tested (in object coordinates). Return the topmost object
		 *            under this point, or if dragging/stretching the whole canvas, just return a
		 *            reference to the canvas.
		 * @return a reference to the object under the point being tested, or null to cancel the
		 *         drag operation. If dragging/stretching the whole canvas (e.g. in a photo viewer),
		 *         always return non-null, otherwise the stretch operation won't work.
		 */
		public T getDraggableObjectAtPoint(PointInfo touchPoint);

		/**
		 * Get the screen coords of the dragged object's origin, and scale multiplier to convert
		 * screen coords to obj coords. The job of this routine is to call the .set() method on the
		 * passed PositionAndScale object to record the initial position and scale of the object (in
		 * object coordinates) before any dragging/stretching takes place.
		 * 
		 * @param obj
		 *            The object being dragged/stretched.
		 * @param objPosAndScaleOut
		 *            Output parameter: You need to call objPosAndScaleOut.set() to record the
		 *            current position and scale of obj.
		 */
		public void getPositionAndScale(T obj, PositionAndScale objPosAndScaleOut);

		/**
		 * Callback to update the position and scale (in object coords) of the currently-dragged
		 * object.
		 * 
		 * @param obj
		 *            The object being dragged/stretched.
		 * @param newObjPosAndScale
		 *            The new position and scale of the object, in object coordinates. Use this to
		 *            move/resize the object before returning.
		 * @param touchPoint
		 *            Info about the current touch point, including multitouch information and
		 *            utilities to calculate and cache multitouch pinch diameter etc. (Note:
		 *            touchPoint is volatile, if you want to keep any fields of touchPoint, you must
		 *            copy them before the method body exits.)
		 * @return true if setting the position and scale of the object was successful, or false if
		 *         the position or scale parameters are out of range for this object.
		 */
		public boolean setPositionAndScale(T obj, PositionAndScale newObjPosAndScale,
				PointInfo touchPoint);

		/**
		 * Select an object at the given point. Can be used to bring the object to top etc. Only
		 * called when first touchpoint goes down, not when multitouch is initiated. Also called
		 * with null on touch-up.
		 * 
		 * @param obj
